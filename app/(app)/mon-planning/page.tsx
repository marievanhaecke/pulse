...
type SessionWithBookings = Session & {
    bookings: { id: string; status: string; user: { full_name: string } }[]
}
...
export default async function MonPlanningPage() {
  ...
  const upcoming = (coachSessions || [])
      .map(sc => sc.session as SessionWithBookings | null)
      .filter((s): s is SessionWithBookings => !!s && s.date >= today && s.status === 'scheduled')
  ...
  return (
        <div className="space-y-8">
              <div>
                      <h1 className="page-title">Mon planning</h1>h1>
                      <p className="text-slate-500 mt-1">{upcoming.length} séance(s) à animer</p>p>
              </div>div>
        
          {/* Upcoming */}
              <div>
                      <h2 className="section-title mb-4">Séances à venir</h2>h2>
                {upcoming.length === 0 ? (
                    <div className="card-padding text-center py-16">
                                <Calendar className="w-10 h-10 mx-auto mb-3 text-slate-300" />
                                <p className="text-slate-400">Aucune séance à venir</p>p>
                    </div>div>
                  ) : (
                                                                      ...
                                                                      <div key={session.id} className="card overflow-hidden">
                                                                                        ...
                                                                                        <div className="p-5">
                                                                                                            <div className="flex items-start justify-between mb-3">
                                                                                                                                  <div>
                                                                                                                                                          <h3 className="font-semibold text-slate-900 text-lg">{session.title}</h3>h3>
                                                                                          ...
                                                                                          {confirmedBookings.length > 0 && (
                                                                                              <div className="border-t border-slate-100 pt-3">
                                                                                                                      <p className="text-xs font-medium text-slate-500 mb-2">Participants :</p>p>
                                                                                          ...
                                                                                          {session.notes && (
                                                                                              <div className="bg-slate-50 rounded-lg p-3 mt-3">
                                                                                                                      <p className="text-sm text-slate-600">{session.notes}</p>p>
                                                                                                </div>div>
                                                                                                            )}
                                                                                          </div>div>
                                                                      </div>div>
          ...
          {past.length > 0 && (
                  <div>
                            <h2 className="section-title mb-4 text-slate-500">Séances passées</h2>h2>
                            <div className="space-y-2">
                              {past.map(session => {
                                  const count = session.bookings?.filter((b) => b.status === 'confirmed').length || 0
                                                  return (
                                                                    <div key={session.id} className="card px-5 py-3 flex items-center gap-4 opacity-70">
                                                                                      ...
                                                                                      <div className="flex-1">
                                                                                                          <p className="font-medium text-slate-700 text-sm">{session.title}</p>p>
                                                                                                          <p className="text-xs text-slate-400">
                                                                                                            {formatDate(session.date)} · {formatTime(session.start_time)}
                                                                                                            </p>p>
                                                                                        </div>div>
                                                                                      <span className="text-xs text-slate-400">{count} participant(s)</span>span>
                                                                    </div>div>
                                                                  )
                              })}
                            </div>div>
